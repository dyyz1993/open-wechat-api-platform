 ...
 * @Filename: api_common.js
 * @Last modified by:   yingzhou xu
 * @Last modified time: 2017-06-17T12:48:35+08:00
 */
 ...
 *
 */
var API = function(appid,getTokenUrl,saveToken, getToken) {
    this.appid = appid;
    this.getTokenUrl = getTokenUrl;
    if(!this.appid || !this.getTokenUrl){
      throw new Error('appid or getTokenUrl is must');
    }
 ...
 */
API.prototype.changeToken = function(data, callback) {
...
API.prototype.getAccessToken = function(callback) {
    var that = this;
    var getTokenUrl = this.getTokenUrl;
    _util.get(getTokenUrl)
        .then(res => {
            res = _util.strToJson(res)
            if(res.code!=0){
              callback(res);
            }else{
              callback(null,res.data)
              that.changeToken(res.data, callback)
            }
        }).catch(err=>{
          callback(err)
        })
...
API.patch = function(functionName, apiUrl, override) {
    ...
    _util.make(API.prototype, functionName, function(info, callback) {
        var hasMark = apiUrl.indexOf('?') >= 0;
        var url = apiUrl + (hasMark ? '&access_token=' : '?access_token=') + this.token.accessToken;
        this.request(url, _util.postJSON(info), wrapper(callback));
    });
};
